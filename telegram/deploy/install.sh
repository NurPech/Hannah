#!/usr/bin/env bash
# install.sh — Hannah Telegram Bot installer / updater
#
...
"${VENV}/bin/pip" install --quiet -r "${INSTALL_DIR}/telegram/requirements.txt"
...
if [[ ! -d "$CONFIG_DIR" ]]; then
    ...
    info "Example config: ${INSTALL_DIR}/telegram/config.yaml"
fi

# ── systemd unit ──────────────────────────────────────────────────────────────
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -f "${SCRIPT_DIR}/hannah-telegram.service" ]]; then
    install -m 644 "${SCRIPT_DIR}/hannah-telegram.service" "$SERVICE_FILE"
else
    install -m 644 "${INSTALL_DIR}/telegram/deploy/hannah-telegram.service" "$SERVICE_FILE"
fi
