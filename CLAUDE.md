...
│   └── config.yaml              ← Infra-/Bootstrap-Config; NLU-Wortlisten, llm.system_prompt, ble.tags und cars liegen in der DB (hannah.db, Settings-Tabelle) statt hier — siehe GetSettings/UpdateConfig gRPC. nlu wird bei leerer DB automatisch mit generischen Defaults befüllt (SettingsManager.seed_defaults(), #114). iobroker.state_names ist seit #257 kein DB-Setting mehr, sondern ein hartkodierter Fallback in core/hannah/iobroker.py (DEFAULT_IOBROKER_STATE_NAMES) für Adapter <1.1.0 (hannah-proto <3.8.0), nicht mehr nutzereditierbar
│
...
| `SubscribeInfrastructure` / `LogCollectorConnect` | Service Discovery (#335): Log-Collector meldet sich per Stream an (Verdrängung wie `ChannelConnect`), Komponenten bekommen Snapshot + Deltas der verfügbaren Infrastruktur-Dienste. Grundlage ist die gemeinsame Component-Registry (`core/hannah/component_registry.py`) |
...
10. **Hannah Core (Cache)**: `IoBrokerClient.handle_state_update()` (`core/hannah/iobroker.py`) übersetzt den rohen State-Suffix über die hartkodierte `DEFAULT_IOBROKER_STATE_NAMES`-Tabelle zurück auf den kanonischen Key, bevor `device.current[canon]` geschrieben wird. **Wichtig:** `AgentStateUpdate` (das Live-Update-Proto) hat kein `canonical_key`-Feld — das existiert nur auf `AgentDevice` (Snapshot/Registrierung, Schritt 6). Live-Updates laufen also **immer** über diese Suffix-Tabelle, unabhängig von Adapter-Version/`common.custom`-Override. **Fehlt der Suffix dort, wird das Update lautlos verworfen** — der Wert bleibt für immer auf dem Stand des letzten Snapshots stehen. Ein `canonicalKey`-Override in ioBroker (siehe Schritt 6) behebt nur die Kategorisierung beim initialen Snapshot, **nicht** dieses Live-Update-Problem — seit #257 gibt es dafür keinen editierbaren Workaround mehr (weder `config.yaml` noch DB); nötig wäre eine Erweiterung von `AgentStateUpdate` um `canonical_key` im Proto.
...
- Neuer/unüblicher Live-Update-Suffix ohne Eintrag in `DEFAULT_IOBROKER_STATE_NAMES` (`core/hannah/iobroker.py`) → Wert friert nach dem ersten Snapshot ein, auch mit korrektem `canonicalKey`-Override in ioBroker (der wirkt nur auf den Snapshot, siehe Schritt 10)
...
8. **Beim Start der Arbeit an einem Issue wird es dem Bearbeiter zugeordnet** (Assignee: `NurPech`, GitLab-Username). Zeigt im Milestone/Board sichtbar "in Bearbeitung" an, unabhängig vom Status des zugehörigen MR.
9. **Wenn die Arbeit für ein Issue fertig ist — bevor daraus ein MR/PR wird — wird das Issue dem rollierenden Milestone `next` (project 319) zugeordnet.** Grund: sobald ein Issue im Milestone `next` steht, ist klar, dass die Umsetzung fertig ist und mit dem nächsten Release ausgeliefert wird; ergänzt die Assignee-Zuordnung aus Punkt 8 (die nur "in Bearbeitung" markiert, nicht "fertig").
10. **Commit Messages bei funktionalen Änderungen referenzieren das Work Item** mit `Refs #ID` (im jeweils zuständigen Tracker aus Punkt 7).
11. **MR/PR-Beschreibungen referenzieren das Work Item nur noch, sie schließen es nicht** (`Refs #ID`, gleiches Format wie Punkt 10). Grund: ohne Release ist eine Änderung nicht im Feld verfügbar — ein Issue vorher zu schließen würde suggerieren, die Lösung sei schon draußen. Erst der Release-MR aus `scripts/release.js` schließt beim eigenen Merge alle zu diesem Zeitpunkt in `next` gesammelten Issues gebündelt (`Closes #id1, #id2, ...`) und benennt `next` danach auf die veröffentlichte Versionsnummer um (Archiv-Charakter), bevor ein frisches leeres `next` für die nächste Sammelrunde entsteht. Gilt nur fürs Monorepo/GitLab — `iobroker.hannah` (GitHub, eigene Release-Pipeline) bleibt vorerst beim alten `Closes #ID` auf dem Feature-/Fix-PR selbst.
12. **Adapter-Changelog (`README.md` in `iobroker.hannah`) enthält keine Issue-/Ticket-Referenzen**, egal welcher Tracker — das sind öffentliche, nutzerseitige Release Notes (npm/ioBroker-Nutzer ohne Zugriff auf interne Tracker), keine internen Entwickler-Notizen.
13. **Der Submodule-Pointer im Hannah-Repo (`iobroker.hannah` — einziges verbliebenes Submodule) zeigt immer auf einen Release-Tag**, nie auf einen Branch-/Feature-Commit. Ablauf: PR im Submodule mergen → Release schneiden (Tag entsteht) → erst dann den Pointer im Hannah-Repo auf diesen Tag bumpen (eigener, fokussierter Commit, getrennt von der eigentlichen Feature-Arbeit). **Dabei immer auch das `branch`-Feld in `.gitmodules` auf denselben Tag aktualisieren** — sonst hält Renovate den Pointer für veraltet (vergleicht gegen `.gitmodules`) und versucht ihn auf den alten Tag zurückzudrehen.

Punkte 7–11 gelten für funktionale Änderungen (Features, Bugfixes) — nicht für reine Doku-/Chore-Änderungen wie diesen Abschnitt selbst.
