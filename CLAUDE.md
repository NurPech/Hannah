...
9. **MR/PR-Beschreibungen referenzieren das Work Item nur noch, sie schließen es nicht** (`Refs #ID`, gleiches Format wie Punkt 8). Grund: ohne Release ist eine Änderung nicht im Feld verfügbar — ein Issue vorher zu schließen würde suggerieren, die Lösung sei schon draußen. Stattdessen wird das Issue beim Merge dem rollierenden GitLab-Milestone `next` (project 319) zugeordnet. Erst der Release-MR aus `scripts/release.js` schließt beim eigenen Merge alle zu diesem Zeitpunkt in `next` gesammelten Issues gebündelt (`Closes #id1, #id2, ...`) und benennt `next` danach auf die veröffentlichte Versionsnummer um (Archiv-Charakter), bevor ein frisches leeres `next` für die nächste Sammelrunde entsteht. Gilt nur fürs Monorepo/GitLab — `iobroker.hannah` (GitHub, eigene Release-Pipeline) bleibt vorerst beim alten `Closes #ID` auf dem Feature-/Fix-PR selbst.
